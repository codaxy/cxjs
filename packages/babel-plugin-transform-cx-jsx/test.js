...
describe("JSCX", function () {
   ...
   it("allows multiple spreads", function () {
      let Component = {};
      let test = { a: 1, b: 2 };
      let test2 = { a: 1, b: 2 };

      let code = `<cx><Component {...test} {...test2} /></cx>`;
      ...
      assert.deepEqual(eval(output), { $type: Component, jsxSpread: [test, test2] });
   });
   ...
   it("converts only inner content of Cx elements", function () {
      ...
      assert.equal(
         ...
         '<Cx items={[{ $type: Container, class: "test", jsxAttributes: ["class"]}]}></Cx>'
      );
   });
   ...
   it("import for HtmlElement is automatically added", function () {
      ...
      assert.equal(
         unwrap(output).replace(/\n/g, "").replace(/\s\s/g, " "),
         ['import { HtmlElement } from "cx/widgets";', 'let x = { $type: HtmlElement, tag: "div"}'].join("")
      );
   });
