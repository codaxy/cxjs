...
export class WheelComponent extends VDOM.Component<WheelComponentProps, WheelComponentState> {
   ...
   UNSAFE_componentWillReceiveProps(props: WheelComponentProps): void {
      ...
      // For a cyclic wheel the option list is the value range repeated, so the
      // same value appears in several copies. Snap to the copy nearest the
      // wheel's current position rather than the one `props.index` names — a
      // wrap then scrolls one item into the adjacent (identical) copy with no
      // jump, instead of animating all the way back to the centre copy.
      if (props.cycle) {
