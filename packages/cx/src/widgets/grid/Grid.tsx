...
export type GridFetchRecordsResult<T> =
  | T[]
  | { records: T[]; lastPage?: boolean; totalRecordCount?: number };
...
export interface GridConfig<T = any> extends StyledContainerConfig {
  ...
  dataAdapter?:
    | CreateConfig<typeof DataAdapter>
    | CreateConfig<typeof ArrayAdapter>
    | CreateConfig<typeof TreeAdapter>
    | CreateConfig<typeof GroupAdapter>;
  ...
  onDragOver?:
    | string
    | ((e: GridDragEvent<T>, instance: Instance) => void | boolean);

  onRowDropTest?: string | ((e: DragEvent, instance: Instance) => boolean);
  onRowDragOver?:
    | string
    | ((e: GridRowDragEvent<T>, instance: Instance) => void | boolean);
  onRowDrop?:
    | string
    | ((e: GridRowDragEvent<T>, instance: Instance) => void | boolean);
  ...
  onColumnDrop?:
    | string
    | ((e: GridColumnDropEvent, instance: Instance) => void);
  ...
  onRowContextMenu?:
    | string
    | ((e: React.MouseEvent<any>, instance: Instance) => void);

  /** Callback function to be executed when a column header is right-clicked.  */
  onColumnContextMenu?:
    | string
    | ((e: React.MouseEvent<any>, columnInstance: Instance) => void);
  ...
  onBeforeCellEdit?:
    | string
    | ((change: GridCellBeforeEditInfo, record: DataAdapterRecord<T>) => any);

  /** A callback function which is executed after a cell has been successfully edited. */
  onCellEdited?:
    | string
    | ((change: GridCellEditInfo<T>, record: DataAdapterRecord<T>) => void);

  /** A callback function which is executed after a column has been resized. */
  onColumnResize?: (
    data: { width: number; column: Config },
    instance: Instance,
  ) => void;
  ...
  onGetColumns?: (
    params: any,
    instance: Instance,
  ) => GridColumnConfig[] | GridRowConfig;
  ...
  onTrackMappedRecords?:
    | string
    | ((records: DataAdapterRecord<T>[], instance: Instance) => void);
...
> {
  ...
  declare grouping?:
    | string
    | GridGroupingConfig<any>
    | (string | GridGroupingConfig<any>)[];
      ...
      line.items.forEach((c: any, index: number) => {
        ...
        if (c.mergeCells) {
          if (row.header.items.length > 1)
            Console.warn(
              "Merged columns are only supported in grids in which rows have only one line of cells.",
            );
          else {
  ...
  prepareData(context: RenderingContext, instance: GridInstance) {
    ...
    if (
      instance.cache("columnParams", data.columnParams) ||
      groupingChanged ||
      !row
    ) {
      row = instance.row = this.createRowTemplate(
        context,
        data.columnParams,
        instance,
        grouping,
    ...
    let skipDefaultSorting =
      this.clearableSort && instance.state.disableDefaultSort;
    if (
      !skipDefaultSorting &&
    ...
    if (sortField) {
      for (let l = 1; l < 10; l++) {
        ...
        let sortColumn =
          line &&
          line.columns &&
          line.columns.find((c: any) => c.field == sortField);
        if (sortColumn) {
    ...
    if (this.onCreateIsRecordDraggable) {
      instance.isRecordDraggable = instance.invoke(
        "onCreateIsRecordDraggable",
        null,
        instance,
      );
  ...
  resolveGrouping(grouping: any) {
    if (grouping) {
      if (!isArray(grouping)) {
        if (isString(grouping) || isObject(grouping)) grouping = [grouping];
          ...
          throw new Error(
            "Dynamic grouping should be an array of grouping objects.",
          );
      }
    ...
    let showHeader =
      !isArray(grouping) || !grouping.some((g: any) => g.showHeader);
    ...
    return (
      <div
        ...
        onMouseDown={(e) => {
          ...
          let resizeOverlayEl: HTMLDivElement | null =
            document.createElement("div");

          let gridEl = (e.target as HTMLElement).parentElement!.parentElement!
            .parentElement!.parentElement!.parentElement!.parentElement!;
          let uniqueColId = (e.currentTarget as HTMLElement).dataset
            .uniqueColId;
          ...
          resizeOverlayEl.style.left = `${
            (headerCell as HTMLElement).getBoundingClientRect().left -
          ...
          captureMouse2(e, {
            onMouseMove: (e) => {
              ...
              let width = Math.max(
                30,
                Math.round(
                  initialWidth + cursor.clientX - initialPosition.clientX,
                ),
              );
            ...
            onDblClick: () => {
              ...
              let tableClone: HTMLTableElement = table.cloneNode(
                true,
              ) as HTMLTableElement;
  ...
  renderHeader(
    ...
    header.children.forEach((line: any) => {
      ...
      line.children.forEach((hdinst: any, colIndex: number) => {
        ...
        for (let l = 0; l < 3; l++) {
          ...
          if (header) {
            ...
            let uniqueColumnId =
              header.data.colSpan > 1 ? null : hdwidget.uniqueColumnId;
            ...
            if (
              (hdwidget.resizable || header.data.resizable) &&
            ...
            if (colIndex > 0) {
              ...
              if (
                ...
                prevColumnResizer = this.renderResizer(
                  instance,
                  hdinstPrev,
                  headerPrev,
                  colIndex - 1,
          ...
          result[l].push(
            <th
              ...
              onMouseMove={(e) =>
                this.onHeaderMouseMove(e, hdwidget, hdinst, instance, l)
              }
      ...
      if (result[0]) {
        ...
        headerRows.push(
          ...result.map((h, i) => <tr key={`${lineIndex}-${i}`}>{h}</tr>),
        );
      }
    });
      ...
      initiateDragDrop(
        e,
        {
          ...
          clone: {
            ...
            matchWidth: true,
            widget: () => (
              <div className={CSS.element(baseClass, "col-header-drag-clone")}>
                {data.text}
              </div>
            ),
          },
  ...
  mapRecord(
    ...
    return instance.dataAdapter.mapRecord(
      context,
      instance,
      data,
      instance.store,
...
> {
  ...
  createRowRenderer(cellWrap: any) {
    ...
    let { widget, isRecordSelectable, visibleColumns, isRecordDraggable, row } =
      instance;
    let { CSS, baseClass } = widget;
    let { dragSource } = data;
    let {
  ...
  render() {
    ...
    if (widget.buffered) {
      ...
      if (hasMergedCells) {
        ...
        let getCellRenderInfo = (vdom: any, cellIndex: number) =>
          vdom.content[0]?.content[cellIndex];
              ...
              if (prevRow?.type == "data") {
                ...
                switch (mergedCol.mode) {
                  case "always":
                    ...
                    break;
                  case "same-value":
                    let prevCellInfo = getCellRenderInfo(
                      prevRow.vdom,
                      mergedCol.index,
                    );
                    shouldMerge =
        ...
        <tbody key="dropzone">
          <tr>
            <td
              ...
              style={{
                height:
                  data.dropMode == "insertion" ? 0 : this.state.dropItemHeight,
              }}
            />
          </tr>
        </tbody>
    ...
    let shouldRenderFixedFooter =
      widget.scrollable && (fixedFooter || fixedColumnsFixedFooter);
  ...
  onScroll() {
    ...
    if (widget.buffered && !this.pending) {
      ...
      if (widget.measureRowHeights && instance.records)
        start = Math.max(
          0,
          this.estimateStart(instance.records, this.dom.scroller!.scrollTop) -
            widget.bufferStep,
        );
  ...
  onDrop(e: DragEvent) {
    try {
      ...
      let { dropInsertionIndex, dropTarget, dropNextToTheRowAbove } =
        this.state;
  ...
  onDropTest(e: DragEvent) {
    ...
    let row =
      widget.onRowDropTest && instance.invoke("onRowDropTest", e, instance);
    let column =
      widget.onColumnDropTest &&
  ...
  onDragMeasure(e: DragEvent, { test: { grid, row, column } }: any) {
    ...
    let r = getTopLevelBoundingClientRect(
      column ? this.dom.el! : this.dom.scroller!,
    );
  ...
  onDragOver(
    ev: DragEvent,
    { test: { grid, row, column } }: GridDropOperationContext,
  ) {
    ...
    if (rowOverIndex != null) {
      ...
      if (
        widget.onRowDragOver &&
        instance.invoke("onRowDragOver", evt, instance) === false
      )
        cancel = true;
      else if (
        rowOverIndex != this.state.dropInsertionIndex ||
      ...
      if (
        ...
        s != this.state.dropInsertionIndex ||
        this.state.dropTarget != "grid"
      ) {
        this.setState({
        ...
        for (let c = 0; c < sr.children.length; c++) {
          ...
          if ((cell as HTMLElement).dataset.uniqueColId)
            instance.state.lockedColWidth[
              (cell as HTMLElement).dataset.uniqueColId!
            ] = (sr.children[c] as HTMLElement).offsetWidth;
        }
      }
    }
    ...
    if (widget.scrollable) {
      ...
      if (this.dom.fixedHeader!) {
        let fixedHeaderTBody = this.dom.fixedHeader!.firstChild!
          .firstChild as HTMLTableSectionElement;
      ...
      if (this.dom.fixedColumnsFixedHeader!) {
        ...
        let fixedHeaderTBody = this.dom.fixedColumnsFixedHeader!
          .firstElementChild?.firstElementChild as HTMLTableSectionElement;
        ...
        if (this.dom.fixedColumnsFixedFooter!) {
          ...
          if (dstTableBody) {
            let srcTableBody = this.dom.fixedTable!
              .lastElementChild as HTMLTableSectionElement;
      ...
      if (widget.buffered) {
        ...
        if (widget.measureRowHeights) {
          ...
          for (let i = 0; i < this.dom.table!.children.length; i++) {
            ...
            if (body.dataset.recordKey != null) {
              ...
              if (!(body.dataset.recordKey! in this.rowHeights))
                this.heightStats.add(
                  body.dataset.groupElement!,
                  body.offsetHeight,
                );
            }
          }
        ...
        if (widget.measureRowHeights && instance.records) {
          ...
          lowerMargin = this.estimateHeight(
            instance.records,
            end,
      ...
      if (widget.scrollSelectionIntoView && !widget.buffered) {
        ...
        let firstSelectedRow =
          this.dom.table!.querySelector(selectedRowSelector);
            ...
            if (item) {
              if (widget.cellEditable)
                if (
                  ...
                  let cell =
                    fixedItem &&
                    (fixedItem.firstChild as HTMLElement)!.children[
                      this.state.cursorCellIndex
                    ];
  ...
  beginDragDrop(e: React.MouseEvent, record: MappedGridRecord) {
    ...
    let add = (
      ...
      let mappedRecord = rec
        ? { ...rec }
        : (widget.mapRecord(null!, instance, data, index) as MappedGridRecord);
