   ...
   initInstance(context, instance) {
      instance.store = new DataProxyView({
         ...
         onSet: (path, value) => {
            ...
            if (!config.set) throw new Error(`Cannot set value for ${path} in DataProxy as the setter is not defined.`);
            ...
            if (isString(config.set)) instance.getControllerMethod(config.set)(value, instance);
               ...
               throw new Error(
                  `Cannot set value for ${path} in DataProxy as the setter is neither a function or a controller method.`
               );
...
class DataProxyView extends ReadOnlyDataView {
   ...
   setItem(path, value) {
      ...
      if (!isObject(this.privateData) || !this.privateData.hasOwnProperty(bindingRoot)) {
         if (isUndefined(value)) return super.deleteItem(path);
