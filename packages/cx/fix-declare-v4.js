    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (inClass && !inInterface && !inReactComponent) {
            ...
            if (match && !trimmed.includes('declare')) {
                ...
                // Skip if it's a method (has opening parenthesis)
                ...
                while (lookAhead < lines.length) {
                    checkLine = lines[lookAhead];

                    // Check for initializer in this line
                    let depth = 0;
                    for (let j = 0; j < checkLine.length; j++) {
                        const char = checkLine[j];
                        if (char === '<' || char === '{' || char === '[' || char === '(') depth++;
                        if (char === '>' || char === '}' || char === ']' || char === ')') depth--;
                        if (char === '=' && depth === 0 && !checkLine.substring(0, j).includes('=>')) {
                            hasInitializer = true;
                            break;
                        }
                    }

                    if (hasInitializer) break;

                    // Stop if we hit a semicolon at depth 0
