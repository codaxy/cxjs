...
- An accessor chain passed to a field's `help` or `label`, or to `LabeledContainer`'s `label`, no longer crashes rendering with `ins.scheduleExploreIfVisible is not a function`. Every property read on an accessor returns another accessor, so the check for a widget class mistook it for one; only a real component type now counts, and the accessor becomes the bound text, like `bind(...)` ([#1333](https://github.com/codaxy/cxjs/issues/1333))
- A controller attached to a `Window` or another overlay can now close it with `this.instance.parentOptions.dismiss()`. Previously only code inside the overlay received its `dismiss` ([#1332](https://github.com/codaxy/cxjs/issues/1332))
...
- `Grid` rows are no longer emitted as `tr` elements directly under `table`. Rows of a grid with merged cells are rendered as bare `tr`, because a `rowspan` cannot cross a row group boundary, but nothing enclosed them — so the rendered tree did not match the described one, React reported a hydration mismatch, and serializing the DOM produced one row group per row with the row's own `class` and `data-record-key` hoisted onto it. Consecutive such rows now share a generated `tbody` ([#1328](https://github.com/codaxy/cxjs/issues/1328))
- `Grid` column headers now render inside `thead` instead of `tbody`. Headers repeated for each group through `grouping.showHeader` keep the `tbody` tag, as a second `thead` is not allowed between the data rows
- `Grid` group captions now carry `role="columnheader"`. A caption is a heading for the rows it introduces rather than a value in the table, but it was exposed as an ordinary cell, so nothing associated it with those rows — a screen reader announced it as data, and accessibility checkers flagged the row as irregular, including when validating a tagged PDF export against PDF/UA. In the aggregate caption row (`grouping.showCaption`) only the cells that come from a column `caption` are marked; aggregate values stay plain cells ([#1329](https://github.com/codaxy/cxjs/issues/1329))
- `Grid` looked its rows up as `tbody[data-record-key]` when scrolling the cursor into view, which silently matched nothing in a grid with merged cells
...
- `AggregateFunction` is now exported from `cx/data`. The class backing the grid's `sum`, `avg`, `count`, `distinct`, `min`, `max` and `last` aggregates was reachable only through a deep import
...
- `Svg` now forwards `role` and `ariaLabel` to the rendered `svg` element, defaulting `role` to `img` when `ariaLabel` is set. Naming a drawing announces it as a single graphic instead of as its individual shapes, which is also how a tagged PDF export tags it. An unnamed drawing is left untouched ([#1327](https://github.com/codaxy/cxjs/pull/1327))
- `Legend` entry shapes are now hidden from assistive technology by default — the shape is a color key whose meaning the entry text beside it already carries, so it is no longer announced as an unnamed graphic. Added `shapeRole` and `shapeAriaLabel` for callers that need the shape exposed and named ([#1327](https://github.com/codaxy/cxjs/pull/1327))
- `TextArea` now implements the `trim` flag it inherits from `TextField`. Trimming applies to the commit that ends the edit, not to intermediate `input` commits, so a trailing newline can still be typed; whitespace inside the text is never touched ([#1321](https://github.com/codaxy/cxjs/pull/1321))
- Added two Vite plugins as counterparts of the existing Babel and webpack tooling ([#1318](https://github.com/codaxy/cxjs/pull/1318)):
  - `vite-plugin-transform-cx-imports` rewrites namespace imports (`import { Grid } from "cx/widgets"`) into direct file imports, for faster dev-server cold starts and deterministic tree shaking. It detects a pre-bundled `cx` and skips the rewrite where it would break resolution
  - `vite-plugin-cx-scss-manifest` collects the CxJS modules an app actually imports and generates an SCSS manifest, so only those widget styles are compiled — which can cut the generated CSS by 70–90% in smaller apps. Deep imports are collected from Vite's module resolution at no cost; pairing it with the plugin above turns barrel imports into deep imports before it sees them, removing the source parsing entirely

**Fixes**

- Dropdowns, `LookupField` option lists and context menus now position correctly when the element they belong to lives inside a same-origin iframe. Placement is computed against the document the dropdown actually renders in, mouse-tracked placements translate the cursor into that document's coordinate space, and scroll and resize of every frame window between the related element and the top document are now observed, so the dropdown follows its field instead of drifting. Viewport measurement also switched from `offsetWidth`/`offsetHeight` to `clientWidth`/`clientHeight`, so scrollbars no longer count as available space ([#1325](https://github.com/codaxy/cxjs/pull/1325))
- Focus detection now looks through same-origin iframes. `getActiveElement` takes an optional `document` and drills into the focused frame's own document instead of stopping at the `<iframe>` element, `isSelfOrDescendant` walks out through ancestor frame elements, and `Window`, `MenuItem` and the `DOM` focus helpers use it in place of `document.activeElement` ([#1325](https://github.com/codaxy/cxjs/pull/1325), [#1326](https://github.com/codaxy/cxjs/pull/1326))
- `Overlay` now clears the resize cursor when the pointer leaves a resizable edge, restoring the cursor from the widget's own style instead of leaving the last `*-resize` value stuck ([#1322](https://github.com/codaxy/cxjs/pull/1322))
- Type-only re-exports (`GridRowConfig`, `AccessorChain`, and the `JSX` namespace in `jsx-dev-runtime`) now use `export type`, so compiling CxJS sources with isolated-modules transpilers no longer fails ([#1318](https://github.com/codaxy/cxjs/pull/1318))
- `cx-build-tools`' manifest recorder now resolves re-exports that declare a source, so re-exported entries are recorded against the module they actually come from ([#1318](https://github.com/codaxy/cxjs/pull/1318))
...
- `LineGraph` smoothing now uses monotone cubic interpolation. Curves rendered with `smooth` follow the data without ever overshooting its vertical range — flat segments stay flat and steep transitions no longer produce phantom peaks or dips, for both line and area rendering. The previous bezier-based smoothing has been removed, and `smoothingRatio` is deprecated and ignored, as monotone interpolation has no configurable curvature ([#1312](https://github.com/codaxy/cxjs/pull/1312))
- `Grid` columns that define an explicit `sortField` now sort by it even when they also display a computed `value`. The sort key precedence is now `sortValue` > `sortField` > `value` > `field`, and the rule is documented on the `sortField` and `sortValue` column properties ([#1317](https://github.com/codaxy/cxjs/pull/1317))

**Fixes**

- Fixed sorting of `Grid` columns identified by a value selector instead of a field. Clicking such a column no longer crashes when the `sortField`/`sortDirection` bindings are declared but empty, sorted-header indicators and sort toggling now identify a sort by its field and value selector pair — so field-less columns no longer light up or toggle together, and columns sharing a field name no longer hijack each other's sort direction ([#1316](https://github.com/codaxy/cxjs/pull/1316))
- `NumericScale.trackValue` now constrains the computed value instead of the raw pixel coordinate, matching `TimeScale`. Previously, with a pixel range wider than the value range, an in-range value whose pixel fell outside the scale bounds was wrongly clamped ([#1311](https://github.com/codaxy/cxjs/pull/1311))
- Widgets defined in the `ui` namespace (`Widget`, `PureContainer`, `Repeater`, layout components, etc.) are now re-exported from `cx/widgets` using direct `export ... from` declarations instead of being imported and then re-exported, which improves tree-shaking
...
- Added the missing `arrowIcon` prop to `TreeNodeConfig`, so setting the expand/collapse glyph from JSX no longer raises a type error ([#1309](https://github.com/codaxy/cxjs/pull/1309))
...
- Projects must update their SCSS entry points — see [Breaking Changes](/docs/intro/breaking-changes#2631---modern-sass-modules) for migration instructions

**Features**

- New [`cx-theme-variables`](https://www.npmjs.com/package/cx-theme-variables) theme based entirely on CSS custom properties, enabling runtime theme switching. Try it out in the [Theme Editor](/themes).
