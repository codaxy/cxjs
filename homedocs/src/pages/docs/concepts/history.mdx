...
| `History.removeNavigateConfirmation()`                 | Removes the registered navigation confirmation, e.g. once the changes are saved.                                                                                                 |
...
The confirmation is asked for link navigation, for programmatic navigation through `pushState`, and for browser
Back and Forward navigation.

Browsers apply Back and Forward navigation before the application is notified, so it cannot be prevented. The
router instead returns the browser to the entry which is displayed as soon as it learns about the navigation,
and travels to the target only if the user confirms. This keeps the address bar in sync with the page while the
confirmation is pending.

Leaving the application entirely (closing the tab or entering a different address) is not covered by this
mechanism and requires a `beforeunload` handler.
