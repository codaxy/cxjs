...
export default class extends Controller {
   init() {
      ...
      if (auth) {
         ...
         if (auth.expiry_time > new Date().getTime()) this.store.set("user.email", auth.email);
      }
      ...
      if (!this.store.get("fiddle") && !this.store.get("qs.f")) {
         ...
         if (lastFiddle) {
            try {
               ...
               if (fiddle.accessCode) History.replaceState({}, null, Url.resolve("~/?f=" + fiddle.accessCode));
            } catch (e) {}
         }
      }
      ...
      this.addTrigger(
         "autoImport",
         ["fiddle.js", "autoImport"],
         (js, autoImport) => {
            if (autoImport) this.store.update("fiddle.js", addMissingImports);
   ...
   publish(e) {
      var fiddle = this.store.get("fiddle");
      if (!fiddle.owned) throw new Error("Internal error: Cannot publish fiddles you do not own.");
   ...
   unpublish(e) {
      var fiddle = this.store.get("fiddle");
      if (!fiddle.owned) throw new Error("Internal error: Cannot unpublish fiddles you do not own.");
      ...
      var interval = setInterval(() => {
         try {
            if (loginWindow.document.domain === document.domain && loginWindow.document.readyState == "complete") {
               ...
               if (result) {
                  var params = queryString.parse(loginWindow.document.location.search);
                  clearInterval(interval);
                  var redirect_uri = Url.absolute("~/?auth=1&provider=" + params.provider);
                  getToken(params.provider, params.state, params.code, redirect_uri)
                     .then((x) => {
                        close();
