...
import { Grid, Pagination, Section, Select, TextField } from "cx/widgets";
         ...
         (size, page, sorters, filter) => {
            ...
            setTimeout(() => {
               ...
               if (filter) {
                  ...
                  if (filter.phone) filtered = filtered.filter((x) => x.phone.indexOf(filter.phone) != -1);

                  if (filter.city) filtered = filtered.filter((x) => x.city.indexOf(filter.city) != -1);
               }
...
export default (
   <cx>
      <a
         href="https://github.com/codaxy/cx/tree/master/gallery/routes/general/grids/filtering.tsx"
         target="_blank"
         putInto="github"
      >
         Source Code
            ...
            columns={[
               {
                  ...
                  header2: {
                     allowSorting: false,
                     items: (
                        <TextField value={bind("$page.filter.name")} style="width:100%" placeholder="Filter by Name" />
                     ),
                  },
               },
               {
               ...
               },
               {
                  ...
                  header2: {
                     allowSorting: false,
                     items: (
                        <TextField value={bind("$page.filter.city")} style="width:100%" placeholder="Filter by City" />
                     ),
                  },
                  field: "city",
                  sortable: true,
