...
export const LookupFields = <cx>
   <Md>
      ...
      * User-friendly multiple selection mode
      ...
      Most of these features were implemented following the excellent [select2 jQuery plugin](https://select2.github.io/).
      ...
      It's important to remember how to properly bind data to the `LookupField` widget. If `multiple` option is not specified
      (single selection mode) then it's required to bind `value` and `text` properties.

      In multiple selection mode, it's necessary to bind the `records` or `values` property. The `records` property will hold a list of
      selected options. By default, only `id` and `text` properties are copied from the option to the selection;
      however, it's possible to provide a list of `bindings` which describes the mapping between options and value fields.
